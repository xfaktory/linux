...
#define EBC_COUNT_SIZE(ebc)		((ebc)->pixels / 1) /* 8bpp */
...
#define EBC_LUT_MAX_PHASES		256

#define EBC_NUM_SUPPLIES		3

#define PIXELS_PER_U64			16
...
struct rockchip_ebc {
	struct clk			*dclk;
	struct clk			*hclk;
	struct completion		refresh_complete;
...
static int lut_type = DRM_EPD_WF_GC16;
...
static bool rockchip_ebc_generate_frame(struct rockchip_ebc *ebc,
					...
					u8 global_count)
{
	u8 *count_buffer = ebc->count_buffer;
	u64 *frame_buffer = ebc->frame_buffer;
	u64 *prev_buffer = ebc->prev_buffer;
	u64 *next_buffer = ebc->next_buffer;
	unsigned int pixel, stride, x, y;
	bool ret = false;

	/* Operate on 8-byte groups of 4-bit pixels. */
	stride = ebc->panel.width / PIXELS_PER_U64;

	for (y = 0; y < ebc->panel.height; y++) {
		for (x = 0; x < stride; x++) {
			unsigned int pos = y * stride + x;
			u64 fbpx = frame_buffer[pos];
			u64 prev = prev_buffer[pos];
			u64 next = next_buffer[pos];
			u32 output = 0;

			if (global_count || fbpx != next || next != prev) {
				bool dirty = false;

				for (pixel = 0; pixel < PIXELS_PER_U64; pixel++) {
					u64 shift = 4 * pixel;
					u64 mask = 0xfUL << shift;
					u64   fb_pixel = fbpx & mask;
					u64 prev_pixel = prev & mask;
					u64 next_pixel = next & mask;
					u8 count = global_count ?: count_buffer[PIXELS_PER_U64 * pos + pixel];
					u8 this_frame;

					/* If no waveform in progress, check for framebuffer updates. */
					if (!count) {
						if (fb_pixel == next_pixel)
							continue;
						next ^= next_pixel ^ fb_pixel;
						next_pixel = fb_pixel;
						dirty = true;
						count = ebc->lut.num_phases;
					}

					this_frame = ebc->lut.num_phases - count;
					output |= ebc->lut.lut[this_frame << 8 |
							       next_pixel >> shift << 4 |
							       prev_pixel >> shift] << (2 * pixel);

					/* Update the previous buffer when the waveform completes. */
					if (!--count) {
						prev ^= prev_pixel ^ next_pixel;
						dirty = true;
					}
					/* Save the decremented frame counter. */
					if (!global_count)
						count_buffer[PIXELS_PER_U64 * pos + pixel] = count;
				 ...
				 bool global_refresh)
{
	...
	if (ret < 0)
		...
		if (ret < 0)
			DRM_DEV_ERROR(dev, "Failed to set temperature to %d: %d\n",
				      temperature, ret);
	}
	...
	global_count = global_refresh ? ebc->lut.num_phases : 0;
	DRM_DEV_DEBUG_DRIVER(dev, "Starting refresh: global=%d waveform=%d temp=%d -> frames=%d\n",
			     global_refresh, waveform, ebc->temperature, ebc->lut.num_phases);
	...
	while (rockchip_ebc_generate_frame(ebc, ebc->dma_buffer + offset, global_count)) {
		...
		if (frames)
			wait_for_completion_timeout(&ebc->refresh_complete,
						    msecs_to_jiffies(50));

		reinit_completion(&ebc->refresh_complete);
...
static void rockchip_ebc_refresh_work(struct work_struct *work)
{
	...
	case EBC_PANEL_INIT:
		...
		rockchip_ebc_refresh(ebc, DRM_EPD_WF_RESET, true);
		ebc->panel_state = EBC_PANEL_ON;
		break;
	case EBC_PANEL_ON:
		if (force_refresh) {
			force_refresh = false;
			rockchip_ebc_refresh(ebc, DRM_EPD_WF_GC16, true);
		}
		break;
	}
...
static void rockchip_ebc_pipe_enable(struct drm_simple_display_pipe *pipe,
...
static void rockchip_ebc_pipe_disable(struct drm_simple_display_pipe *pipe)
{
	struct rockchip_ebc *ebc = pipe_to_ebc(pipe);
	struct device *dev = ebc->drm.dev;
...
static u8 rockchip_ebc_apply_damage(struct rockchip_ebc *ebc, void *vaddr,
				    struct drm_rect *clip)
{
	...
	for (y = clip->y1; y < clip->y2; y++) {
		u32 *src = vaddr + 4 * ebc->panel.width * y;
		u8 *dst = ebc->frame_buffer + ebc->panel.width / 2 * (ebc->panel.height - y);
				     ...
				     struct drm_plane_state *old_plane_state)
{
	...
	map = &to_drm_shadow_plane_state(plane_state)->data[0];

	drm_atomic_helper_damage_iter_init(&iter, old_plane_state, plane_state);
	drm_atomic_for_each_plane_damage(&iter, &clip) {
		if (rockchip_ebc_apply_damage(ebc, map->vaddr, &clip))
			need_refresh = true;
	}
...
static const struct drm_simple_display_pipe_funcs rockchip_ebc_pipe_funcs = {
	.enable		= rockchip_ebc_pipe_enable,
	.disable	= rockchip_ebc_pipe_disable,
...
static int rockchip_ebc_probe(struct platform_device *pdev)
{
	...
	init_completion(&ebc->refresh_complete);
	...
	ret = drmm_epd_lut_init(drm, &ebc->lut, "waveform.bin",
				DRM_EPD_LUT_4BIT, EBC_LUT_MAX_PHASES);
	if (ret)
		goto err_disable_pm;
	...
	/* Limited by the window register field widths. */
