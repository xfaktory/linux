				 ...
				 bool global_refresh)
{
	...
	DRM_DEV_INFO(dev, "Starting refresh: global=%d mode=%d temp=%d -> frames=%d\n",
	...
	DRM_DEV_INFO(dev, "Finished refresh: frames=%d\n", frames);
}
...
static int rockchip_ebc_probe(struct platform_device *pdev)
{
	...
	ebc->count_buffer = devm_kmalloc(dev, EBC_COUNT_SIZE(ebc), GFP_KERNEL);
	ebc->frame_buffer = devm_kmalloc(dev, EBC_FRAME_SIZE(ebc), GFP_KERNEL);
