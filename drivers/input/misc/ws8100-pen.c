...
static void ws8100_pen_input(struct spi_device *spi)
{
	...
	dev_dbg(&spi->dev, "Got input 0x%02x\n", input_status);

	switch (input_status) {
	case 0x01: /* short press */
		key = KEY_APPSELECT;
		break;
	case 0x08: /* double press */
		key = KEY_ASSISTANT;
		break;
	case 0x10: /* long press */
		key = KEY_SLEEP;
		break;

	case 0x02: /* short press */
		btn = BTN_TOOL_RUBBER;
		key = KEY_PAGEDOWN;
		break;
	case 0x20: /* long press */
		btn = BTN_TOOL_RUBBER;
		key = KEY_FORWARD;
		break;
	case 0x22: /* double press */
		btn = BTN_TOOL_RUBBER;
		key = KEY_UNDO;
		break;
	case 0x23: /* leading edge */
		btn = BTN_TOOL_RUBBER;
		break;

	case 0x04: /* short press */
		btn = BTN_TOOL_PEN;
		key = KEY_PAGEUP;
		break;
	case 0x40: /* long press */
		btn = BTN_TOOL_PEN;
		key = KEY_BACK;
		break;
	case 0x42: /* double press */
		btn = BTN_TOOL_PEN;
		key = KEY_ONSCREEN_KEYBOARD;
		break;
	case 0x43: /* leading edge */
		btn = BTN_TOOL_PEN;
		break;

	default:
		dev_warn(&spi->dev, "Unxpected input: 0x%02x\n", input_status);
		return;
	}

	if (key) {
		input_report_key(pen->input, key, 1);
		input_sync(pen->input);
		input_report_key(pen->input, key, 0);
		if (btn)
			input_report_key(pen->input, btn, 0);
	} else {
		if (btn)
			input_report_key(pen->input, btn, 1);
	}
	input_sync(pen->input);
}
...
static const enum power_supply_property ws8100_pen_psy_properties[] = {
...
static int ws8100_pen_psy_get_proprty(struct power_supply *psy,
				      enum power_supply_property psp,
				      union power_supply_propval *val)
{
	struct spi_device *spi = power_supply_get_drvdata(psy);
	struct ws8100_pen *pen = spi_get_drvdata(spi);
	ktime_t now = ktime_get();
	int ret;


	if (ktime_after(now, pen->pen_battery_time)) {
		ret = ws8100_pen_get_pen_battery(spi);
		if (ret && ret != -ENXIO)
			return ret;

		pen->pen_battery_time = ktime_add_ns(now, NSEC_PER_SEC);
		pen->pen_present = !ret;
	}

	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
		val->intval = pen->pen_battery[0]
			? POWER_SUPPLY_STATUS_CHARGING
			: POWER_SUPPLY_STATUS_DISCHARGING;
		return 0;
	case POWER_SUPPLY_PROP_PRESENT:
		val->intval = pen->pen_present;
		return 0;
	case POWER_SUPPLY_PROP_CAPACITY:
		val->intval = pen->pen_battery[1];
		return 0;
...
static const struct power_supply_desc ws8100_pen_psy_desc = {
	.name			= "ws8100_pen",
	.type			= POWER_SUPPLY_TYPE_BATTERY,
	.properties		= ws8100_pen_psy_properties,
	.num_properties		= ARRAY_SIZE(ws8100_pen_psy_properties),
	.get_property		= ws8100_pen_psy_get_proprty,
};
...
{
	struct power_supply_config psy_cfg = { .drv_data = spi };
	...
	pen->input->name = "ws8100_pen";
	pen->input->phys = "ws8100_pen/input0";
	pen->input->id.bustype = BUS_BLUETOOTH;

	input_set_capability(pen->input, EV_KEY, KEY_APPSELECT);
	input_set_capability(pen->input, EV_KEY, KEY_ASSISTANT);
	input_set_capability(pen->input, EV_KEY, KEY_SLEEP);
	input_set_capability(pen->input, EV_KEY, BTN_TOOL_RUBBER);
	input_set_capability(pen->input, EV_KEY, KEY_PAGEDOWN);
	input_set_capability(pen->input, EV_KEY, KEY_FORWARD);
	input_set_capability(pen->input, EV_KEY, KEY_UNDO);
	input_set_capability(pen->input, EV_KEY, BTN_TOOL_PEN);
	input_set_capability(pen->input, EV_KEY, KEY_PAGEUP);
	input_set_capability(pen->input, EV_KEY, KEY_BACK);
	input_set_capability(pen->input, EV_KEY, KEY_ONSCREEN_KEYBOARD);

	ret = input_register_device(pen->input);
	if (ret)
		return dev_err_probe(dev, ret, "Failed to register input\n");

	psy = devm_power_supply_register(dev, &ws8100_pen_psy_desc, &psy_cfg);
	if (IS_ERR(psy))
		return PTR_ERR(psy);

	power_supply_powers(psy, &pen->input->dev);
